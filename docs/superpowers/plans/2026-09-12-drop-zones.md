...
- Feature id `FeatureID("dropzones")`, title "Drop Zones". Settings keys exactly: `dropzones.airdrop`, `dropzones.stash`, `dropzones.secondZone`, `dropzones.stashDropAction` (values `replace` | `add`, default `replace`).
...
- The drop catcher is an ordinary-space window (never added to the private SkyLight space); the island keeps drawing the zones. The catcher is ordered in only while zones are shown.
