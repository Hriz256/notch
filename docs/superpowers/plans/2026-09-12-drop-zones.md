...
- The drop catcher is an ordinary-space window (never added to the private SkyLight space) and is what *draws* the zones panel as well as receiving the drop — the island's private space composites above the system's drag image, and moving the window into the user space does not change that (measured on the user's Mac, macOS 26.5). The island is suppressed to the bare notch while the panel is up. The catcher is ordered in only while zones are shown, plus one 0.45 s collapse animation.
