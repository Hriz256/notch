...
- The drop catcher is an ordinary-space window (never added to the private SkyLight space), invisible (`alphaValue 0`) and only there to receive the drop; the island keeps drawing the zones. The catcher is ordered in only while zones are shown.
