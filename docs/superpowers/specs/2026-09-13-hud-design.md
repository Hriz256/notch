...
**Date:** 2026-09-13
**Status:** draft for the user's review (per-section review waived, as for Drop Zones)
...
| What is shown | A `Presentation` with `priority: .alert`, `style: .peek`, `expanded: nil`, `showsStackDots: false`, `ttl: nil` (the view model owns the timer so updates can re-arm it). Leading slot = symbol + label; trailing slot = the bar. Nothing else about the island changes; when the HUD ends the island returns to whatever card it was showing. |
| Copy and symbols | Brightness: `sun.max.fill`, label **"Brightness"**. Sound: label **"Sound"**; symbol `speaker.slash.fill` when muted or at 0, `speaker.wave.1.fill` below ⅓, `speaker.wave.2.fill` below ⅔, else `speaker.wave.3.fill`. Both 13 pt semibold white, symbol 12 pt, 6 pt apart, leading-aligned in the slot. (Seam's exact copy; symbols chosen to match its glyphs.) |
...
| Timing | The presentation appears on the first change and is dismissed **1.5 s** after the *last* change (`holdDuration`); every further change of the same kind calls `update` and re-arms the timer. A change of the *other* kind while one is up dismisses it and presents the new one at once (one HUD at a time, like the system). Appear and dismiss animations are the island's standard alert-peek animations. |
...
| Brightness source | `DisplayServices.framework` (private), loaded with `dlopen`/`dlsym` at runtime: `DisplayServicesRegisterForBrightnessChangeNotifications(displayID, callback)` and `DisplayServicesGetBrightness(displayID, &value)` for the **built-in display** (`CGDisplayIsBuiltin`) only. If any symbol is missing, or the display cannot change brightness (`DisplayServicesCanChangeBrightness`), brightness is silently unavailable and logged once; the volume half keeps working. |
| Suppressing the system HUD | `SystemHUDSuppressor`, applied while the feature is active and at least one kind is on: 1) `EnableSystemBanners = false` in `com.apple.controlcenter` (via `CFPreferencesSetAppValue` + `CFPreferencesAppSynchronize`), 2) restart Control Center (`SIGTERM` to the `ControlCenter` process; launchd relaunches it), 3) `launchctl kickstart -k gui/<uid>/com.apple.OSDUIHelper`, then `SIGSTOP` to `OSDUIHelper`. A **watchdog** every 5 s (only while applied) re-does step 3 if there is no `OSDUIHelper` process in the stopped state. Lifted on deactivate, on the master switch going off, and in `applicationWillTerminate`: `SIGCONT`, the preference removed *only if we set it* (a user who already had it `false` keeps it), Control Center restarted. `hud.suppressionApplied` in `UserDefaults` records that we applied it, so a crash or `kill -9` is repaired at the next launch: if the flag is set and the feature is off, the suppressor lifts; if on, it re-applies. Restarting Control Center blinks the menu bar for a moment; it happens only when the feature is toggled, never on a plain launch when the preference is already in the wanted state. |
| Known gaps (documented, accepted) | A media key at an already-maximal or minimal level changes nothing, so no HUD appears (the system would have shown one). While suppressed, `OSDUIHelper`'s other bezels (Caps Lock, keyboard backlight, eject) do not appear either. |
...
public enum SuppressionStep: Equatable, Sendable {
    ...
    case continueOSDUIHelper           // SIGCONT
}
...
- **`SystemHUDSuppressor`**: executes `SuppressionStep`s through a `SystemShell` protocol (`setControlCenterBanners(Bool?)`, `restartControlCenter()`, `kickstartOSDUIHelper()`, `signalOSDUIHelper(SIGSTOP/SIGCONT)`, `isOSDUIHelperStopped() -> Bool`, `bannersPreference() -> Bool?`). `apply()` / `lift()` / the 5 s watchdog on an `IslandClock`. Persists `hud.suppressionApplied` and `hud.weSetBannersPreference` in `UserDefaults`. The real shell: `CFPreferences` for the preference; `kill(pid, SIGTERM/SIGSTOP/SIGCONT)` on pids found with `proc_listpids` (no `killall` subprocess); `launchctl kickstart` via `Process`; stopped-state check via `proc_pidinfo` (`PROC_PIDTBSDINFO`, `pbi_status == SSTOP`).
