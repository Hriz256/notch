...
| Copy and symbols | Brightness: `sun.max.fill`, label **"Brightness"**. Sound: label **"Sound"**; symbol `speaker.slash.fill` when muted or at 0, `speaker.wave.1.fill` below ⅓, `speaker.wave.2.fill` below ⅔, else `speaker.wave.3.fill`. Both 13 pt semibold white, symbol 12 pt, 6 pt apart, leading-aligned in the slot. The symbol sits in a fixed 14 pt box so the label does not shift as `speaker.wave.N` changes width. The glyph **morphs** rather than swaps when it changes — `.contentTransition(.symbolEffect(.replace.downUp))` driven by `.spring(response: 0.2, dampingFraction: 1.0)`, which a replace effect needs or it cuts — and under Reduce Motion it swaps plainly (`.identity`, no animation), not the cross-fade `.replace` degrades to on its own (*added 2026-09-13, audit B3*). The 14 pt box stays outside the transition. (Seam's exact copy; symbols chosen to match its glyphs.) |
| The bar | Trailing slot: a capsule **60 × 4 pt**, track white at 25 %, fill white, width = level. The fill animates `.spring(response: 0.24, dampingFraction: 0.72)` — it runs slightly past the new level and settles, and the track clips the overshoot at full volume (*changed 2026-09-13 from `.easeOut(duration: 0.12)`, audit B8 — owner's call: juicier than macOS, whose own bar does not overshoot*). Reduce Motion drops the animation. Muted draws the fill at 0. |
...
| Brightness source | `DisplayServices.framework` (private), loaded with `dlopen`/`dlsym` at runtime: `DisplayServicesRegisterForBrightnessChangeNotifications(displayID, observer, callback)` — verified by spike on 2026-09-13: three arguments, the callback is a `CFNotificationCallback` whose `userInfo` carries `"value"`, and it fires on CoreBrightness's own XPC queue, *not* on the registering thread, so the callback must `DispatchQueue.main.async` before it touches anything main-actor (assuming main isolation inside it traps) — and `DisplayServicesGetBrightness(displayID, &value)` for the **built-in display** (`CGDisplayIsBuiltin`) only. The notification carries no source: the ambient light sensor's adjustments arrive through it too, as ramps of small steps every 8 ms (~0.0001 a step when gentle, up to 0.004 a step and 0.3/s when the sensor is covered outright), while the keys jump 1/16 (1/64 with Option+Shift) in one notification and land on that grid (measured 2026-09-14). `BrightnessChangeClassifier` keeps only changes that are both a single step of at least 3/4 of a fine key step *and* on the 1/64 grid — speed is deliberately not a criterion, a strong ambient change is as fast as any smoothed key press — so the HUD, like the system's, stays silent while the sensor works. If any symbol is missing, or the display cannot change brightness (`DisplayServicesCanChangeBrightness`), brightness is silently unavailable and logged once; the volume half keeps working. |
...
- **`SystemHUDSuppressor`**: executes `SuppressionStep`s through a `SystemShell` protocol (`bannersPreference() -> Bool?`, `setBannersPreference(Bool?)`, `restartControlCenter() -> Bool`, `kickstartOSDUIHelper()`, `stopOSDUIHelper()`, `isOSDUIHelperStopped() -> Bool`), off the main actor (the steps block for a few seconds at most). `apply()` / `lift()` are `async`; the 5 s watchdog runs on an `IslandClock`. Persists `hud.suppressionApplied`, `hud.weSetBannersPreference` and `hud.controlCenterConfigured` in `UserDefaults`; the last is set only when a restart step actually reported success, and cleared only by a lift that restarted Control Center. The real shell: `CFPreferences` for the preference; signals we send ourselves for both restarts (`SIGTERM` to Control Center, which launchd relaunches; `SIGKILL` to the helper followed by `launchctl kickstart gui/<uid>/com.apple.OSDUIHelper` via `Process`, without `-k` — SIP refuses `-k` for these agents with exit 150 — each waiting up to a second for the signalled pid to go, and reporting whether the Control Center restart happened at all); `kill(pid, SIGSTOP)` on the pid found with `proc_listpids` + `proc_pidpath` (retrying for up to a second after the kickstart, the helper takes a moment to appear); stopped-state check via `proc_pidinfo` (`PROC_PIDTBSDINFO`, `pbi_status == SSTOP`).
