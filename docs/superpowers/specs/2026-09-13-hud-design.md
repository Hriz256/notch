...
public enum SuppressionStep: Equatable, Sendable {
    ...
    case restartControlCenter          // launchctl kickstart -k gui/<uid>/com.apple.controlcenter
    case kickstartOSDUIHelper          // launchctl kickstart -k gui/<uid>/com.apple.OSDUIHelper
...
- **`SystemHUDSuppressor`**: executes `SuppressionStep`s through a `SystemShell` protocol (`bannersPreference() -> Bool?`, `setBannersPreference(Bool?)`, `restartControlCenter()`, `kickstartOSDUIHelper()`, `stopOSDUIHelper()`, `isOSDUIHelperStopped() -> Bool`), off the main actor (the steps block for up to a second). `apply()` / `lift()` are `async`; the 5 s watchdog runs on an `IslandClock`. Persists `hud.suppressionApplied` and `hud.weSetBannersPreference` in `UserDefaults`. The real shell: `CFPreferences` for the preference; `launchctl kickstart -k` via `Process` for both restarts; `kill(pid, SIGSTOP)` on the pid found with `proc_listpids` + `proc_pidpath` (retrying for up to a second after the kickstart, the helper takes a moment to appear); stopped-state check via `proc_pidinfo` (`PROC_PIDTBSDINFO`, `pbi_status == SSTOP`).
