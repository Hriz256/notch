...
| Drop on stash | Add (default) or replace per `stashDropAction`; the third zone does the other action. Files are **copied** into `~/Library/Application Support/Notch/Stash/<UUID>/<name>` (original untouched); the index `~/Library/Application Support/Notch/stash.json` holds `[StashedFile]` + `stashedAt`. TTL **86 400 s** from `stashedAt`, checked on load and armed as a one-shot timer while running; expiry (`StashStore.expire()`) deletes the folders the index points at plus `stash.json` and then sweeps, so a file detached minutes ago keeps its own hour — the "Clear stash" row's `clear()` is the brutal one and takes all of `Stash/`. A `stash.json` that cannot be decoded is replaced with an empty index and its orphaned folders are swept on the same rule, an hour old and up; originals are never touched. |
...
| Settle | After a stash drop the panel shows a single full-width stash card headed "N File(s)" with the thumbnails entering (0.4 s), then the zones presentation is dismissed (island collapses) and the stash peek shows. *Changed 2026-09-13 (audit A8 + B9): the settle is a spring rather than an easeOut and each tile lands ~40 ms behind the one in front of it, newest first — the tiles are caught, not parked. The 0.4 s window itself is unchanged: the stagger is budgeted against it, so the last tile *starts* early enough to be still before the island collapses. The hover-expanded row is budgeted the other way — 0.2 s caps when its last box **starts**, not when the row is finished.* |
...
| Drag out | The thumbnail stack (peek and expanded) is an `NSDraggingSource`: one `NSFilePromiseProvider` per stashed file (delegate copies the stashed file to the destination), all files in one session, drag image = cascade of the files' QuickLook thumbnails, each fitted into 48×48 keeping its aspect ratio (the system icon at 32×32 only when QuickLook has no preview), offset (+4, −4) per item, `sourceOperationMask` = `.copy` outside the app / `[.copy, .generic, .move]` inside. A session that ends with a non-empty operation **clears the stash** with a poof (Seam's observed behaviour: after dragging out, the island moved on to the next card); a cancelled session leaves it. Every item also carries **`public.file-url`** for the stored copy, after the promise types — Finder redeems the promise, while Chromium/Electron apps, Telegram and file inputs read the URL and would otherwise refuse the drop. Nothing on screen waits for the promises: when the poof is over every file in the session leaves the shelf through `StashStore.detach(fileIDs:)` — index rewritten, card dismissed when it is empty — and only the *bytes* wait. A promise redeemed within `promiseSettleTimeout` (5 s) means the receiver has its own copy, so that folder is deleted (`StashStore.deleteDetached(fileID:)`); an unredeemed one means the receiver took the URL and may still be reading ours, so the folder stays and is swept once it is `StashStore.orphanLifetime` (1 h) old — measured from the **detach**, which dates the folder — by a later `load()` or by the one sweep each detach arms on the island's clock (`orphanSweepDelay`, `StashStore.sweepOrphans()`). |
| Drag out, one file | Each tile in the expanded row is its own drag source for its own file (one promise plus the same `public.file-url`, that file's own thumbnail as the drag image). Both kinds of drag-out mark `dragOutPhase = .dragging`, so the panel offers the stash card alone either way; `dragOutEnded(completed:files:)` takes a `DragOutScope` (`.all` — the default, and the only one that empties the stash — or `.single(UUID)`). A completed `.single` poofs that tile alone (fade + scale over 0.25 s) and then takes just that file off the shelf by the same two steps as a whole-stash drag-out — `detach` now, folder deleted or left to the sweep once its promise has settled — while the rest of the pile keeps its card, its id and its `stashedAt`. The last file leaving dismisses the stash presentation exactly as a completed whole-stash drag-out does. Each tile's context menu also carries a "Remove `<name>`" row through the same path. |
...
- Animations: geometry via the existing `TransitionChoreographer`; targeting/width changes `.spring(response: 0.3, dampingFraction: 0.8)`; cards appear `.opacity + .scale(0.95)` on `contentIn`; dropped thumbnails enter with scale 1.12 → 1 per tile on `.spring(response: 0.4, dampingFraction: 0.75)`, staggered newest-first (nominally 40 ms, shortened when the boxes would not fit their window — `SettleMotion`); the hover-expanded row's tiles fan in the same way from scale 0.92 + opacity 0, its step shortened so the **last box starts within 0.2 s** (end to end the row's fan-in is about 0.5 s, since the last box still takes its own settle after that); poof = scale → 0.6 + opacity → 0 over 0.25 s. Reduce Motion: no scale/rotation animations, opacity only (the tiles fade in together, unstaggered).
