...
| Known gap: the drag image | The island's private SkyLight Space (absolute level 400) composites above the system's drag-image window, so the thumbnail in the user's hand is hidden *behind* the zones panel; Seam shows it on top. Moving the island window into the active user Space for the length of the drag (`SLSGetActiveSpace` + `SLSSpaceAddWindowsAndRemoveFromSpaces`) was tried on the user's Mac (macOS 26.5) and did **not** change it, so that mechanism was removed. Drawing the panel from a second, ordinary-Space window was tried too and rejected: the panel then reads as a separate shape rather than one growing out of the notch. **Open.** |
...
| Stash card | `.background` peek, sticky: leading = thumbnail stack, trailing = count circle. Hover-expanded: same width, **124 pt** tall: the same two slots on top, then a **row of the stashed files** as individual 40×40 tiles, then a caption row `tray.fill` + "N file(s) · <size>". |
| Stash row (**departs from Seam**, at the user's request) | Seam's expanded card shows the same fanned stack as the peek; pulling one particular file out of a pile of overlapping tiles is guesswork, so the expanded card lays the files out in one row instead — newest leftmost (`index.files` reversed), 40 pt tiles 8 pt apart, centred, 8 pt below the peek row. Hovering a tile lifts it 6 pt and scales it 1.06 (`.spring(response: 0.25, dampingFraction: 0.7)`); Reduce Motion drops both and dims the unhovered tiles to 0.85 instead. At most **six boxes**: six tiles, or five and a "+N" chip (same 40×40 box, 12 pt semibold blue on `Palette.cardFill`, not draggable, not a target). Six is what the card's width holds — the card is deliberately no wider than the peek, because its top row *is* the peek and hovering must not move it. No scrolling. The compact fan in the peek is unchanged. |
...
- Hover-expanded (124 pt tall): top row identical to the peek (same positions, which is why the card is never wider than the peek); 8 pt below it the row of 40 pt tiles (`cornerRadius` 6, 8 pt apart, centred, newest leftmost, hover lift 6 pt / scale 1.06, at most six boxes with a "+N" chip as the sixth past six files); caption row centred 24 pt below the tiles' bottom edge (14 pt gap + a 20 pt row): `tray.fill` 11 pt blue + caption 12 pt medium white 70 %.
