...
Claude events installed: `UserPromptSubmit`, `PreToolUse` (matcher ""), `PostToolUse` (""), `PostToolUseFailure` (""), `PermissionRequest` (""), `PermissionDenied` (""), `Notification`, `Stop`, `SessionEnd`. At launch `reconcile()` re-installs a Claude config that lacks any of these, so an update starts receiving new events without the user toggling the agent. Codex: `UserPromptSubmit`, `PreToolUse` (matcher "*"), `PostToolUse` ("*"), `Stop`. Cursor: `beforeSubmitPrompt`, `preToolUse`, `postToolUse`, `afterFileEdit`, `stop` (timeout 5).
