...
public enum ClaudeUsageParser {
    public static func parse(_ data: Data, now: Date) throws -> AgentUsage {
        ...
        let session = window(limits: limits, kind: "session") ?? window(UsageJSON.object(root["five_hour"]))
        let weekly = window(limits: limits, kind: "weekly_all") ?? window(UsageJSON.object(root["seven_day"]))
    ...
    private static func window(limits: [[String: Any]], kind: String) -> UsageWindow? {
        ...
        return UsageWindow(percent: percent, resetsAt: UsageJSON.date(UsageJSON.value(entry, "resets_at", "resetsAt")))
    }
    ...
    private static func window(_ object: [String: Any]?) -> UsageWindow? {
        ...
        return UsageWindow(percent: percent, resetsAt: UsageJSON.date(UsageJSON.value(object, "resets_at", "resetsAt")))
    }
}
